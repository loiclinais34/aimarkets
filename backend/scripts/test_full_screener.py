#!/usr/bin/env python3
"""
Script de test pour le screener complet
"""
import requests
import time
import json
...
def test_full_screener():
    print("🚀 Test du screener complet...")
    ...
    try:
        ...
        print("📤 Lancement du screener complet...")
        response = requests.post(f"{BASE_URL}/run-full", json=params)
        response.raise_for_status()
        task_info = response.json()
        task_id = task_info.get("task_id")
        print(f"✅ Screener complet lancé. Task ID: {task_id}")
        
        # Attendre la completion
        print("⏳ Attente de la completion...")
        max_attempts = 60  # Plus de temps pour le screener complet
        for attempt in range(max_attempts):
            ...
            if status['state'] == 'SUCCESS':
                print("🎉 Screener complet terminé avec succès!")
                ...
                print(f"❌ Screener complet échoué: {status.get('status', 'Erreur inconnue')}")
                return False
        
        print("⏰ Timeout: Le screener complet n'a pas terminé dans les temps")
        return False
...
if __name__ == "__main__":
    success = test_full_screener()
    if success:
        print("\n🎉 Test du screener complet réussi!")
    else:
        print("\n❌ Test du screener complet échoué.")
