#!/usr/bin/env python3
"""
Script robuste pour calculer TOUS les indicateurs techniques
Version qui gère mieux les valeurs manquantes
"""
...
def calculate_robust_indicators(symbols: list, limit_per_symbol: int = 200):
    """Calculer TOUS les indicateurs techniques de manière robuste"""
    ...
    for i, symbol in enumerate(symbols, 1):
        try:
            ...
            # Nettoyer les données - garder toutes les lignes mais remplacer les infinis
            df = df.replace([np.inf, -np.inf], np.nan)
            
            # Supprimer seulement la colonne temporaire
            df = df.drop('tr', axis=1)
            
            print(f"   ✅ {len(df)} enregistrements après calcul (tous conservés)")
...
def main():
    ...
    calculate_robust_indicators(symbols)
    end_time = time.time()
